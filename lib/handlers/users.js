...
const _users = {
  ...
  async delete (data, callback) {
    ...
    const phone = pathParts.length > 1 && pathParts[1]
    if (!phone) {
    ...
    try {
      ...
      for (const check of user.checks) {
        await _data.remove('checks', check.id).catch(e => console.warn('Could not delete check'))
      }
      await _data.remove('users', phone).catch(e => {
