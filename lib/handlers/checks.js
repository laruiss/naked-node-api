...
const _checks = {
  ...
  async delete(data, callback) {
    ...
    try {
      const check = await _data.read('checks', id).catch(e => { throw new Error('The specified check does not exist')})
      const user = await _data.read('users', token.phone).catch(e => { throw new Error('The user does not exist')})
      user.checks = user.checks.filter(check => check.id !== id)
      await _data.update('users', user.phone, user).catch(e => { throw new Error('Unable to update user') })
      await _data.remove('checks', id).catch(e => { throw new Error('Unable to delete check') })
      callback(200)
    } catch (error) {
      callback(400, { error: error.message, originalError: error })
    }

    // Return a 400 status if no
    if (!(id && token.phone && token.phone.length === 10)) {
      callback(400, { error: 'Missing required fields' })
    }
